...
const FeedNews: React.FC = (props) => {
    ...
    return (
        <View className="flex divide-y-[0.3px] divide-gray-600 px-2">
            <View>
                <Text className="text-lg font-medium">Notícias</Text>
            </View>
            <View>
                <ScrollView showsHorizontalScrollIndicator={false} className="py-3 gap-x-2 shadow-black shadow-2xl" horizontal={true}>
                    {categoryNews.map((item, index) => (
                        <TouchableOpacity disabled={item.name == category} onPress={() => setCategoryFeed(item.name)} key={index} className={`${category == item.name ? 'bg-background-primary' : 'bg-background-light/40'} rounded-md px-4 py-0.5 transition-colors `}>
                            <Text className={`${category == item.name ? 'text-white font-semibold' : 'font-medium text-neutral-200'}`}>{item.name}</Text>
                        </TouchableOpacity>
                    ))}
                </ScrollView>
                {data.map((news, index) => (
                    <TouchableOpacity key={index} className="my-1 bg-white rounded-lg flex shadow-black shadow-2xl">
                        <View className="flex flex-row">
                            <Image source={{ uri: news.imagePath }} className="w-1/3 rounded-l-lg" />
                            <View className="flex gap-y-2 flex-1 p-2">
                                <View className="flex flex-row items-center justify-between ">
                                    <View>
                                        <View className={`${returnCategoryColor(news.category)} px-1 py-0.5 rounded-md`}><Text className="text-xs text-white font-medium">{news.category}</Text></View>
                                    </View>
                                    <View>
                                ...
                                <View>
                                    <Text numberOfLines={2} ellipsizeMode="tail" className="text-[11px] font-light">
                                        {news.title}
                                    </Text>
                                </View>
                                <View>
                                    <Text className="text-[10px] font-extralight text-neutral-700">{formatDate(news.dateReleased)}</Text>
                                </View>
                            </View>
                        </View>
                    </TouchableOpacity>
